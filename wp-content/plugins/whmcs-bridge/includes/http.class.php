...
class bridgeHttpRequest
{
	...
	function connect($url, $withHeaders=true, $withCookies=false) {
        ...
        curl_setopt($ch, CURLOPT_USERAGENT, 'WHMCSBridge');
        ...
        if ($this->follow && isset ($headers['location']) && $headers['location']) {
            ...
            if (strstr($this->_path, '/store/order') === false && strstr($this->_path, '/password/reset/change')) {
                ...
                if (substr($redir, 0, 1) != '/' && stristr($redir, ':208') === false && substr($redir, 0, 4) != 'http')
                    $redir = '/' . $redir;

                $redir_parts = pathinfo($redir, PATHINFO_DIRNAME);
                ...
                || (
                    ...
                    && stristr($redir, 'account/paymentmethods') === false
                ) {
            ...
            if (strstr($redir, 'viewinvoice.php') || (strstr($this->_path, '/store/order') && strstr($redir, 'cart.php'))) {
                ...
                strstr($redir, 'custom_page=manage_validation') || (strstr($url, 'dologin') !== false && !isset($this->post['bg']))
            ) {
                $newRedir =  cc_whmcs_bridge_parse_url($redir);
