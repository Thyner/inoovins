...
define("CC_WHMCS_BRIDGE_VERSION","5.9");
...
function cc_whmcs_admin_notices() {
    ...
    if (cc_whmcs_bridge_mainpage()) {
        ...
        if (phpversion() < '5')
            $warnings[]="You are running PHP version ".phpversion().". We recommend you upgrade to PHP 5.6 or higher.";
...
function cc_whmcs_bridge_checks() {
    ...
    $return_errors = array(
        ...
        'http_codes' => "<span style='color:blue'>Details of response codes:"
                ."<ul><li style='padding-left:20px;'><u>Codes 301/302</u> A redirect is occurring. The most common reason for this is that you have entered different "
                ."URL to the one in your 'System URL' of your 'General Settings' page of WHMCS Admin.<br>Please note that the URL in your WHMCS Bridge settings must "
                ."match *exactly* the URL in your WHMCS Admin. If one has 'www' but the other does not, it will cause problems, if one has http and the other https, "
                ."it will also cause problems.<br>The safest option is to copy and paste it from WHMCS Admin into your WHMCS Bridge Settings.<br>Note: If the 302 is on your index.php but your clientarea.php is returning 200 then this message can be safely ignored as you have set your client area to require a login.</li>"
            ."<li style='padding-left:20px;'><u>Code of 403</u> Please make sure you have allowed your WordPress IP (returned in the error message) to access your API as well as "
            ...
            ."<li style='padding-left:20px; color:green'><u>Code of 200</u> The bridge was able to reach the URL.</li></ul></span>",
        ...
        'password_reset' => 'Since WHMCS 7.5.1, WHMCS has changed the way the password reset page works and our addon for WHMCS is unable to redirect correctly, if you have bridgewp '
            .'enabled <a target="_blank" href="https://i-plugins.com/whmcs-bridge/knowledgebase/1098/Iandsharp039m-using-WHMCS-751-or-higher-with-BridgeWP-Addon-and-the-redirect-is-not-working.html/">please see this KB article</a>.',
    ...
    if ($proceed) {
        ...
        curl_setopt($ch, CURLOPT_URL, $whmcs_url.'/clientarea.php'); // set url to post to
        $data = curl_exec($ch); // run the whole process
        $code2 = curl_getinfo($ch, CURLINFO_HTTP_CODE);

        curl_setopt($ch, CURLOPT_URL, $whmcs_url.'/index.php?systpl=six'); // set url to post to
        $data = curl_exec($ch); // run the whole process
        $code3 = curl_getinfo($ch, CURLINFO_HTTP_CODE);

        if ($code2 != 200 || $code3 != 200) {
            $return_message[] = "Your WHMCS clientarea.php page is returning an HTTP response code of <u>{$code2}</u>."
                    ."<br>Your WHMCS index.php page is returning an HTTP response code of <u>{$code3}</u>.";
            $codekey = true;
            $proceed = false;
        }
        ...
        if (defined("CC_WHMCS_BRIDGE_SSO_PLUGIN") && class_exists('WhmcsProLicense') && class_exists('whmcs')) {
            ...
            curl_setopt($ch, CURLOPT_URL, $whmcs_url.'/pwreset.php?key=123');
            $data = curl_exec($ch);
            $code = curl_getinfo($ch, CURLINFO_HTTP_CODE);

            if ($code != 302 && $code != 301) {
                curl_setopt($ch, CURLOPT_URL, $whmcs_url.'/password/reset/use/key/12345');
                $data = curl_exec($ch);
                $code = curl_getinfo($ch, CURLINFO_HTTP_CODE);
...
function cc_whmcs_bridge_http($page="index") {
    ...
    if ($page=='verifyimage') {
        ...
        $http = $whmcs . $page;
    } elseif (substr($page, -8) == 'download' || stristr($page, 'two-factor') !== false) {
        $http=$whmcs.$page;
    } else {
        $http=$whmcs.$page.'.php';
    }
